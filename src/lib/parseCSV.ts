    ...
    function processChunk() {
      ...
      for (; i < end; i++) {
        ...
        if (isBybit) {
          ...
          // Bybit timestamps are nanoseconds (19 digits) → divide by 1e6 for ms
          // Some files may use ms (13 digits) — handle both
          if (t > 1e15) t = Math.round(t / 1_000_000);  // nanoseconds
          else if (t > 1e13) t = Math.round(t / 1000);  // microseconds
